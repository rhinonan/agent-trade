...
export const analyze = (config: {
  agent: AgentMatch | { id?: string; capability?: string };
  prompt: string;
}): WorkflowStep => ({
  id: "", type: "analyze", ...config,
}) as WorkflowStep;

export const critique = (config: {
  reviewer: string;
  targetStep: string;
  prompt?: string;
}): WorkflowStep => ({
  id: "", type: "critique",
  agent: { id: config.reviewer },
...
// Flat primitive helpers for inline use with parallel/sequential
export const panel = (config: {
  match: AgentMatch;
...
export const synthesize = (config: {
  agent: string;
  prompt: string;
}): WorkflowStep => ({
  id: "", type: "synthesize",
...
export const debate = (config: {
  agents: { id: string }[];
  maxRounds?: number;
  prompt: string;
}): WorkflowStep => ({
  id: "", type: "debate",
  agent: config.agents as AgentMatch[],
...
class WorkflowBuilder {
  ...
  step(id: string, primitive: WorkflowStep, overrides?: Partial<WorkflowStep>): this {
    ...
    // Auto-assign IDs to children of parallel/sequential steps
    if (step.children) {
